...
mat_chd1            <- pool2("chd1_A",             "chd1_B")
mat_isw1            <- pool2("isw1_A",             "isw1_B")
mat_isw2            <- pool2("isw2_A",             "isw2_B")
mat_isw1_isw2_chd1  <- pool2("isw1_isw2_chd1_A",   "isw1_isw2_chd1_B")
...
sample_mats <- list(
  ...
  "isw1"            = mat_isw1,
  "isw2"            = mat_isw2,
  "isw1-isw2-chd1"  = mat_isw1_isw2_chd1
)
...
plot_heatmap_block <- function(mat_block, sample_title = NULL, show_title = FALSE,
  ...
  # Titles only on the top row (like your example)
  if (show_title) {
  ...
  # Only keep one legend (occupancy) overall
  if (!show_legend) {
...
plot_bar_block <- function(df_group, gene_order, show_x = FALSE, show_legend = FALSE) {
  ...
  df_group <- df_group %>%
    ...
    mutate(name = factor(name, levels = rev(gene_order)))  # top-to-bottom match heatmap
  ...
  p <- ggplot(df_group, aes(x = nucPositioning, y = name, fill = regularity)) +
    ...
    ylab(NULL) +
    # Make x numbers readable (avoid 0.0e+00 style)
    scale_x_continuous(
...
# Row counts per group for proportional heights (NA-only rows excluded)
...
barplots <- map(group_names, function(g) {
  gene_order <- rownames(blocks_by_sample[[1]][[g]])  # same order across samples/groups from input slicing
  df_group <- df_reg_all %>% filter(group == g)

  # Show axis + legend only for bottom row (axis) and top row (legend) to keep clean
...
for (g in group_names) {
  for (s in names(sample_mats)) {
    ...
    show_title <- (g == head(group_names, 1))     # only top row has titles
    show_x     <- (g == tail(group_names, 1))     # only bottom row shows x labels

    # Only one occupancy legend: bottom-right heatmap
...
# Layout: 3 rows (groups) × 7 columns (bar + 5 samples + guide area)
# The same letter repeated ("G") makes guide_area span all rows.
layout_design <- "
ABCDEFG
HIJKLMG
NOPQRSG
...
# Assign plots in A..S order (G is guide_area)
plots_in_order <- list(
  # Row 1: convergent
  barplots[[group_names[1]]],
  heatmaps[[paste(group_names[1], "wt",             sep="__")]],
  heatmaps[[paste(group_names[1], "chd1",           sep="__")]],
  heatmaps[[paste(group_names[1], "isw1",           sep="__")]],
  heatmaps[[paste(group_names[1], "isw2",           sep="__")]],
  heatmaps[[paste(group_names[1], "isw1-isw2-chd1", sep="__")]],
  guide_area(),

  # Row 2: lonely
  barplots[[group_names[2]]],
  heatmaps[[paste(group_names[2], "wt",             sep="__")]],
  heatmaps[[paste(group_names[2], "chd1",           sep="__")]],
  heatmaps[[paste(group_names[2], "isw1",           sep="__")]],
  heatmaps[[paste(group_names[2], "isw2",           sep="__")]],
  heatmaps[[paste(group_names[2], "isw1-isw2-chd1", sep="__")]],

  # Row 3: tandem
  barplots[[group_names[3]]],
  heatmaps[[paste(group_names[3], "wt",             sep="__")]],
  heatmaps[[paste(group_names[3], "chd1",           sep="__")]],
  heatmaps[[paste(group_names[3], "isw1",           sep="__")]],
  heatmaps[[paste(group_names[3], "isw2",           sep="__")]],
  heatmaps[[paste(group_names[3], "isw1-isw2-chd1", sep="__")]]
)

final_plot <- wrap_plots(plots_in_order, design = layout_design) +
  plot_layout(
    guides = "collect",
    widths = c(1.5, 1.7, 1.7, 1.7, 1.7, 1.7, 0.9),
...
df_summary_plot_chd1         <- summarise_condition("chd1_A",           "chd1_B",           "chd1")
df_summary_plot_isw1         <- summarise_condition("isw1_A",           "isw1_B",           "isw1")
df_summary_plot_isw2         <- summarise_condition("isw2_A",           "isw2_B",           "isw2")
df_summary_plot_iswichd1     <- summarise_condition("isw1_isw2_chd1_A", "isw1_isw2_chd1_B", "isw1.isw2.chd1")


# merge the samples
df_summary <- rbind(df_summary_plot, df_summary_plot_chd1,
                    df_summary_plot_isw1, df_summary_plot_isw2,
                    df_summary_plot_iswichd1)
...
pdf("reg_score_profile_compare.pdf", width = 8, height = 5)
print(ggProfile)
dev.off()
