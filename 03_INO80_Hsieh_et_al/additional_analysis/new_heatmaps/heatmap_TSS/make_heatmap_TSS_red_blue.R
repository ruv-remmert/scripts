...
final_plot <- wrap_plots(plots_in_order, design = layout_design) +
  plot_layout(
    guides  = "collect",
    widths  = c(1.5, 1.5, 1.5, 0.9),
    heights = as.numeric(row_counts)
  ) &
  theme(legend.position = "right")

ggsave(
  "combined_TSS_ggheatmap_WT_A_ino80_A.png",
  plot   = final_plot,
  width  = 16,
  height = 9,
  dpi    = 300
...
pdf("reg_score_profile_compare_TSS_WT_ino80.pdf", width = 20, height = 12)
print(ggProfile)
dev.off()

ggsave(
  "reg_score_profile_compare_TSS_WT_ino80.png",
  plot   = ggProfile,
  width  = 20,
  height = 12,
  dpi    = 300
