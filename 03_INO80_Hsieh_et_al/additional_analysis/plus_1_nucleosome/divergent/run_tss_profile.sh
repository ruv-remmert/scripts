...
# Example: ./run_tss_profile.sh TSS_nuc_regSorted.bed tss_out *.bw
...
# Step 2: Plot profile from matrix (also emit the per-group data table for the R scripts)
plotProfile \
...
# Step 3: Custom R plots — feed them the plotProfile data table, not the raw matrix.gz
Rscript make_TSS_plots.R "${OUTPUT_PREFIX}_profile.tab" "${OUTPUT_PREFIX}_plot.pdf"
Rscript make_tss_plots_individual.R "${OUTPUT_PREFIX}_profile.tab" "${OUTPUT_PREFIX}_individual_plot.pdf"
