...
# --- Helper: plot one MNase concentration group ---
plot_mnase_group <- function(pairs, outfile, title) {

  # collect all values for this group to set shared y axis
  all_group_vals <- unlist(lapply(pairs, function(p) {
    c(avg_profiles[[p["treatment"]]], avg_profiles[[p["control"]]])
  }))
  y_pad  <- (max(all_group_vals) - min(all_group_vals)) * 0.1
  y_low  <- min(all_group_vals) - y_pad
  y_high <- max(all_group_vals) + y_pad

  # two panels side by side (one per pair)
  pdf(outfile, width = 16, height = 6)
  par(mfrow = c(1, length(pairs)),
      mar = c(6, 6, 5, 4),
      cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.4)

  for (i in seq_along(pairs)) {
    treatment <- pairs[[i]]["treatment"]
    control   <- pairs[[i]]["control"]
    ...
    plot(pos, trt_vals, type = "l",
         ...
         ylab = if (i == 1) "MNase fragment density" else "",
         lwd = 2.5,
         main = paste(treatment, "\nvs", control),
         col = "#2E7D5B",
         ylim = c(y_low, y_high))
...
plot_mnase_group(pairs_50U, "profile_monoNucs_50U.pdf", "TSS profile - 50U MNase")
plot_mnase_group(pairs_5U,  "profile_monoNucs_5U.pdf",  "TSS profile - 5U MNase")
