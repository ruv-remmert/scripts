...
# Normalize to upstream baseline (first 50 bins = -1500 to -1000bp)
# This corrects for library size differences without distorting the TSS peak
baseline_bins <- 1:50
...
for (i in seq_along(names.pre)) {
  if (i == 1) {
    plot(pos, avg_profiles[[names.pre[i]]],
         ...
         ylab = "MNase fragment density (relative to upstream baseline)", lwd = 2.5,
