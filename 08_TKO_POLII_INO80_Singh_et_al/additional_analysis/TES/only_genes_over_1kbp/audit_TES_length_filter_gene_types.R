...
summary_table <- summary_table[
  order(
    summary_table$filter_status == "removed_le1000bp",
    -summary_table$Freq,
...
# ----------------------------
# Removal percentage per gene type (TES + TSS combined)
# ----------------------------

combined_summary <- rbind(tes_summary, tss_summary)
combined_summary$pct_removed <- round(
  combined_summary$removed_le1000bp / combined_summary$total * 100, 1
)
combined_summary <- combined_summary[
  order(-combined_summary$pct_removed),
]
...
write.table(
  combined_summary,
  file = "genes_removed_by_TES_gt1000bp_filter_pct_summary.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)

cat("\n--- Removal percentage by gene type (sorted) ---\n")
print(combined_summary)

# ----------------------------
# PNG table of removed genes by gene type (sorted)
# ----------------------------

removed_summary <- summary_table[
  summary_table$filter_status == "removed_le1000bp",
  c("gene_type", "Freq")
]
removed_summary <- removed_summary[order(-removed_summary$Freq), ]
row.names(removed_summary) <- NULL
colnames(removed_summary) <- c("Gene type", "Removed genes")

png("genes_removed_by_TES_gt1000bp_filter_table.png",
    width = 1400, height = 600, res = 200)

par(mar = c(0.5, 0.5, 0.5, 0.5))
plot.new()
grid.table(removed_summary, rows = NULL)

dev.off()

cat("\nSaved: genes_removed_by_TES_gt1000bp_filter_table.png\n")

cat("\nSaved: genes_removed_by_TES_gt1000bp_filter_pct_summary.tsv\n")
cat("Saved: genes_removed_by_TES_gt1000bp_filter.tsv\n")
