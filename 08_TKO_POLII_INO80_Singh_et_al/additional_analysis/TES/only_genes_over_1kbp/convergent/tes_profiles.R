...
# --- Keep only convergent genes ---
convergent_bed <- "TES_regSorted_convergent.bed"
conv <- read.table(convergent_bed, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
conv_genes <- sub("_mRNA$", "", conv[, 4])
keep_conv <- mat_gene %in% conv_genes

cat("Genes before filtering:", nrow(mat), "\n")
cat("Convergent genes:", sum(keep_conv), "\n")
cat("Genes >1000 bp:", sum(keep_genes), "\n")
cat("Convergent genes >1000 bp:", sum(keep_conv & keep_genes), "\n")

mat <- mat[keep_conv & keep_genes, , drop = FALSE]
...
pdf("TES_profile_convergent_gt1000bp.pdf", width=14, height=7)
...
for(i in seq_along(names.pre)) {
  ...
  if(i==1){
    ...
    plot(
      ...
      lwd=2.5,
      main="TES profile - convergent genes (>1000 bp)",
      col=dark2[i],
      ylim=c(y_low,y_high)
    )
...
cat("Saved: TES_profile_convergent_gt1000bp.pdf\n")
...
for(i in seq_along(names.pre)) {
  ...
  outfile <- paste0(
    "TES_profile_convergent_gt1000bp_",
    names.pre[i],
    "_vs_",
    wt_name,
    ".pdf"
...
if (length(missing) > 0) {
  ...
  pdf("TES_profile_rapamycin_summary_convergent_gt1000bp.pdf", width = 12, height = 6)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TES profile - Rapamycin depletion summary (convergent, >1000 bp)",
  ...
  dev.off()
  cat("Saved: TES_profile_rapamycin_summary_convergent_gt1000bp.pdf\n")
}
