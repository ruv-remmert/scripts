...
lone <- read.table(lonely_bed, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
lone_genes <- sub("_mRNA$", "", lone[, 4])
keep_lonely <- mat_gene %in% lone_genes
...
draw_special <- function() {
  ...
  plot(pos.zoom, wt_vals,
       ...
       main = "TES Profile - Isolated (> 1000 bp)",
       ylim = c(y_low, y_high),
...
render_plot(file.path(out.dir, "TES_lonely_special_comparison_gt1000bp"), draw_special, 7, 7)