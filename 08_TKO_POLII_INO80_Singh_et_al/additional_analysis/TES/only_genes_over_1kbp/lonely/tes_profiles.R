...
# --- Keep only lonely genes ---
lonely_bed <- "TES_regSorted_lonely.bed"
lonely <- read.table(lonely_bed, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
lonely_genes <- sub("_mRNA$", "", lonely[, 4])
keep_lonely <- mat_gene %in% lonely_genes

cat("Genes before filtering:", nrow(mat), "\n")
cat("Lonely genes:", sum(keep_lonely), "\n")
cat("Genes >1000 bp:", sum(keep_genes), "\n")
cat("Lonely genes >1000 bp:", sum(keep_lonely & keep_genes), "\n")

mat <- mat[keep_lonely & keep_genes, , drop = FALSE]
...
pdf("TES_profile_lonely_gt1000bp.pdf", width=14, height=7)
...
for(i in seq_along(names.pre)) {
  ...
  if(i==1){
    ...
    plot(
      ...
      lwd=2.5,
      main="TES profile - lonely genes (>1000 bp)",
      col=dark2[i],
      ylim=c(y_low,y_high)
    )
...
cat("Saved: TES_profile_lonely_gt1000bp.pdf\n")
...
if (length(missing) > 0) {
  ...
  pdf("TES_profile_rapamycin_summary_lonely_gt1000bp.pdf", width = 12, height = 6)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TES profile - Rapamycin depletion summary (lonely, >1000 bp)",
  ...
  dev.off()
  cat("Saved: TES_profile_rapamycin_summary_lonely_gt1000bp.pdf\n")
}