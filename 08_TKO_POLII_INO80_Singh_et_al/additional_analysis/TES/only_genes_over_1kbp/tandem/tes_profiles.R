...
# --- Keep only tandem genes ---
tandem_bed <- "TES_regSorted_tandem.bed"
tandem <- read.table(tandem_bed, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
tandem_genes <- sub("_mRNA$", "", tandem[, 4])
keep_tandem <- mat_gene %in% tandem_genes

cat("Genes before filtering:", nrow(mat), "\n")
cat("Tandem genes:", sum(keep_tandem), "\n")
cat("Genes >1000 bp:", sum(keep_genes), "\n")
cat("Tandem genes >1000 bp:", sum(keep_tandem & keep_genes), "\n")

mat <- mat[keep_tandem & keep_genes, , drop = FALSE]
...
pdf("TES_profile_tandem_gt1000bp.pdf", width=14, height=7)
...
for(i in seq_along(names.pre)) {
  ...
  if(i==1){
    ...
    plot(
      ...
      lwd=2.5,
      main="TES profile - tandem genes (>1000 bp)",
      col=dark2[i],
      ylim=c(y_low,y_high)
    )
...
cat("Saved: TES_profile_tandem_gt1000bp.pdf\n")
...
if (length(missing) > 0) {
  ...
  pdf("TES_profile_rapamycin_summary_tandem_gt1000bp.pdf", width = 12, height = 6)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TES profile - Rapamycin depletion summary (tandem, >1000 bp)",
  ...
  dev.off()
  cat("Saved: TES_profile_rapamycin_summary_tandem_gt1000bp.pdf\n")
}