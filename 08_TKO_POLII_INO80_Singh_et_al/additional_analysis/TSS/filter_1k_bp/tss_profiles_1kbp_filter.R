...
pdf("profile_monoNucs_le1000bp.pdf", width = 14, height = 7)
...
for (i in seq_along(names.pre)) {
  if (i == 1) {
    plot(pos, avg_profiles[[names.pre[i]]],
         ...
         lwd = 2.5, main = "TSS profile - all samples (genes <= 1000 bp)",
...
dev.off()
cat("Saved: profile_monoNucs_le1000bp.pdf\n")
...
for (i in seq_along(names.pre)) {
  ...
  outfile <- paste0("profile_monoNucs_le1000bp_", names.pre[i], "_vs_", wt_name, ".pdf")
  ...
  plot(pos, curr_vals,
       ...
       ylab = "MNase fragment density",
       lwd  = 2.5,
       main = paste("TSS profile -", names.pre[i], "vs", wt_name, "(genes <= 1000 bp)"),
       col  = dark2[i],
       ylim = c(y_low, y_high))
...
if (length(missing) > 0) {
  ...
  pdf("profile_monoNucs_rapamycin_summary_le1000bp.pdf", width = 12, height = 6)
  par(mar = c(6, 6, 5, 16), cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.7)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TSS profile - Rapamycin depletion summary (genes <= 1000 bp)",
  ...
  dev.off()
  cat("Saved: profile_monoNucs_rapamycin_summary_le1000bp.pdf\n")
}
