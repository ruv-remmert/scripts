...
# strip "_monoNucs_profile" and "_1.fastq.gz" suffixes
...
# --- Remove convergent/tandem genes > 1000 bp ---
convergent_bed <- read.table(
  "TES_regSorted_convergent.bed",
  sep = "\t",
  header = FALSE,
  stringsAsFactors = FALSE
)

tandem_bed <- read.table(
  "TES_regSorted_tandem.bed",
  sep = "\t",
  header = FALSE,
  stringsAsFactors = FALSE
)

convergent_genes <- convergent_bed[, 4]
tandem_genes <- tandem_bed[, 4]
...
remove_genes <- gene_length > 1000 &
  gene_name %in% group_genes

keep_genes <- !remove_genes

cat("Genes before filtering:", nrow(mat), "\n")
cat("Convergent/tandem genes > 1000 bp removed:", sum(remove_genes), "\n")
...
# Normalize to upstream baseline (first 50 bins = far upstream)
baseline_bins <- 1:50
...
pdf("profile_monoNucs_without_convergent_tandem_gt1000bp.pdf", width = 14, height = 7)
...
for (i in seq_along(names.pre)) {
  if (i == 1) {
    plot(pos, avg_profiles[[names.pre[i]]],
         ...
         lwd = 2.5, main = "TSS profile 1kbp filter convergent + tandem - all samples",
...
dev.off()
cat("Saved: profile_monoNucs_without_convergent_tandem_gt1000bp.pdf\n")
...
for (i in seq_along(names.pre)) {
  ...
  outfile <- paste0("profile_monoNucs_without_convergent_tandem_gt1000bp_", names.pre[i], "_vs_", wt_name, ".pdf")
  ...
  plot(pos, curr_vals,
       ...
       ylab = "MNase fragment density",
       lwd  = 2.5,
       main = paste("TSS profile 1kbp filter convergent + tandem-", names.pre[i], "vs", wt_name),
       col  = dark2[i],
       ylim = c(y_low, y_high))
...
# check all special samples exist
...
if (length(missing) > 0) {
  ...
  pdf("profile_monoNucs_rapamycin_summary_without_convergent_tandem_gt1000bp.pdf", width = 12, height = 6)
  par(mar = c(6, 6, 5, 16), cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.7)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TSS profile  1kbp filter convergent + tandem - Rapamycin depletion summary",
  ...
  dev.off()
  cat("Saved: profile_monoNucs_rapamycin_summary_without_convergent_tandem_gt1000bp.pdf\n")
}
