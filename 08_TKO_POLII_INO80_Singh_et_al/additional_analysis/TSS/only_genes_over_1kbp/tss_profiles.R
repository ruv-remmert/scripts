...
pdf("profile_monoNucs_gt1000bp.pdf", width = 14, height = 7)
...
dev.off()
cat("Saved: profile_monoNucs_gt1000bp.pdf\n")
...
for (i in seq_along(names.pre)) {
  ...
  outfile <- paste0("profile_monoNucs_gt1000bp_", names.pre[i], "_vs_", wt_name, ".pdf")
...
if (length(missing) > 0) {
  ...
  pdf("profile_monoNucs_rapamycin_summary_gt1000bp.pdf", width = 12, height = 6)
  par(mar = c(6, 6, 5, 16), cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.7)
  ...
  dev.off()
  cat("Saved: profile_monoNucs_rapamycin_summary_gt1000bp.pdf\n")
}
