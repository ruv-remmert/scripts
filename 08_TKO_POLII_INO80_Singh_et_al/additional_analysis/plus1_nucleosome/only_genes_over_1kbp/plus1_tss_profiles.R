...
# (Gene length from GTF since TSS bed has 1-bp coords)
# --------------------------------------------------
...
# --------------------------------------------------
# Combined +1 nucleosome TSS profile
# --------------------------------------------------
...
pdf("plus1_TSS_profile_gt1000bp.pdf", width=14, height=7)
...
for(i in seq_along(names.pre)) {
  ...
  if(i==1){
    ...
    plot(
      ...
      lwd=2.5,
      main="+1 nucleosome TSS profile - all samples (genes >1000 bp)",
      col=dark2[i],
      ylim=c(y_low,y_high)
    )
...
cat("Saved: plus1_TSS_profile_gt1000bp.pdf\n")
...
if (length(missing) > 0) {
  ...
  pdf("plus1_TSS_profile_rapamycin_summary_gt1000bp.pdf", width = 12, height = 6)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "+1 nucleosome TSS profile - Rapamycin depletion summary (genes > 1000 bp)",
...
dev.off()
   cat("Saved: plus1_TSS_profile_rapamycin_summary_gt1000bp.pdf\n")
}

# ---- Mutant comparison plot: TKO + TKO_Rpb1 + TKO_Rpb1_Ino80 (no WT) ----
mutant_samples <- c(
  "_TKO_Rep_1",
  "_TKO_Rpb1_Rapamycin_120_min",
  "_TKO_Rpb1_Ino80_Rapamycin_120_min_Rep_1"
...
mutant_colors <- c("#808080", "#CC0000", "#0000CC")
mutant_labels <- c("TKO", "TKO + Rpb1 depletion", "TKO + Rpb1 + Ino80 depletion")

missing_mut <- mutant_samples[!mutant_samples %in% names.pre]
if (length(missing_mut) > 0) {
  cat("WARNING: missing mutant samples:", paste(missing_mut, collapse = ", "), "\n")
  cat("Available names:", paste(names.pre, collapse = ", "), "\n")
} else {

  mutant_vals <- lapply(mutant_samples, function(s) avg_profiles[[s]])

  all_mutant <- unlist(mutant_vals)
  y_pad  <- (max(all_mutant) - min(all_mutant)) * 0.1
  y_low  <- min(all_mutant) - y_pad
  y_high <- max(all_mutant) + y_pad

  pdf("plus1_TSS_profile_mutant_summary_gt1000bp.pdf", width = 12, height = 6)
  par(mar = c(6, 6, 5, 16), cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.7)

  plot(pos, mutant_vals[[1]],
       type = "n",
       xlab = "Distance from TSS (bp)",
       ylab = "MNase fragment density",
       main = "+1 nucleosome TSS profile - Mutant comparison (genes > 1000 bp)",
       ylim = c(y_low, y_high))
  abline(v = 0, lty = 2, col = "grey70")

  polygon(c(pos, rev(pos)),
          c(mutant_vals[[1]], rev(rep(y_low, length(pos)))),
          col = "#D3D3D3",
          border = NA)

  lines(pos, mutant_vals[[1]], col = "#D3D3D3", lwd = 2.5)

  for (i in 2:length(mutant_samples)) {
    lines(pos, mutant_vals[[i]], col = mutant_colors[i], lwd = 2.5)
  }

  legend(downstream + 50, y_high,
         bty    = "n",
         legend = mutant_labels,
         col    = mutant_colors,
         lwd    = 2.5,
         fill   = c("#D3D3D3", NA, NA),
         border = c("#808080", NA, NA),
         cex    = 1.2,
         xpd    = TRUE)

  dev.off()
  cat("Saved: plus1_TSS_profile_mutant_summary_gt1000bp.pdf\n")
}
