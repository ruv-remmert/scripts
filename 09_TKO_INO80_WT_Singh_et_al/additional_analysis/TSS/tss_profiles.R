...
if (length(missing) > 0) {
  ...
  pdf("profile_monoNucs_rapamycin_summary.pdf", width = 12, height = 6)
  par(mar = c(6, 6, 5, 16), cex.axis = 1.3, cex.lab = 1.5, cex.main = 1.7)
  ...
  for (i in seq_along(special_samples)) {
    if (i == 1) {
      plot(pos, special_vals[[i]],
           ...
           ylab = "MNase fragment density",
           lwd  = 2.5,
           main = "TSS profile - Rapamycin depletion summary",
  ...
  dev.off()
  cat("Saved: profile_monoNucs_rapamycin_summary.pdf\n")
}
