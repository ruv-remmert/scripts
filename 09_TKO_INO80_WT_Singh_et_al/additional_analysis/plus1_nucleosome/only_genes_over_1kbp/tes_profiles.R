...
pdf("TSS_profile_gt1000bp.pdf", width=14, height=7)
...
for(i in seq_along(names.pre)) {
  ...
  if(i==1){
    ...
    plot(
      ...
      lwd=2.5,
      main="TSS profile - all samples (genes >1000 bp)",
      col=dark2[i],
      ylim=c(y_low,y_high)
    )
...
cat("Saved: TSS_profile_gt1000bp.pdf\n")
...
special_labels <- c("Wild type", "TKO", "TKO + Ino80 depletion", "WT + Ino80 depletion")
...
if (length(missing) > 0) {
  ...
  pdf("TSS_profile_rapamycin_summary_gt1000bp.pdf", width = 12, height = 6)
  ...
  dev.off()
  cat("Saved: TSS_profile_rapamycin_summary_gt1000bp.pdf\n")
}
