...
tand <- read.table(tandem_bed, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
tand_genes <- sub("_mRNA$", "", tand[, 4])
keep_tandem <- mat_gene %in% tand_genes
...
draw_special <- function() {
  ...
  plot(pos.zoom, tko_vals,
       ...
       main = "TES Profile - Tandem (> 1000 bp)",
       ylim = c(y_low, y_high),
...
render_plot(file.path(out.dir, "TES_tandem_special_comparison_gt1000bp"), draw_special, 7, 7)