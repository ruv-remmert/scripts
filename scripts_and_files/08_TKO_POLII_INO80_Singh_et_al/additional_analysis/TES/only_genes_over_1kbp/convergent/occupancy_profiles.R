...
gtf_file <- "../../../../data/genes_sacCer3.gtf"
...
############################################
# Plot: TKO (grey filled) vs TKO - Pol II vs TKO - Pol II - INO80
############################################

y_low  <- 0
y_high <- max(c(tko_vals, rpb1_vals, ino80_vals))
...
draw_special <- function() {
  ...
  plot(pos.zoom, tko_vals,
       type = "n",
       xlab = NA,
       ylab = "Nucleosome Occupancy",
       main = "TES Profile - Convergent (> 1000 bp)",
       ylim = c(y_low, y_high),
       xlim = c(start.plot, end.plot),
       axes = TRUE)

  polygon(c(pos.zoom, rev(pos.zoom)), c(tko_vals, rep(0, length(pos.zoom))),
          col = fill_color, border = NA)

  lines(pos.zoom, tko_vals, col = fill_color, lwd = 2)
...
render_plot(file.path(out.dir, "TES_convergent_special_comparison_gt1000bp"), draw_special, 7, 7)