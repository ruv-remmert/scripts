...
# Use clean plot labels on the left and the exact header labels on the right.
# The third entry includes the Ino80-labelled header as a fallback because that
# is the name shown in the supplied sample list.
...
sample_mats <- list(
  ...
  "Wild_type_Rep1" = getHeatSamp(
    sample_label_Wild_type_Rep1
  ),

  "TKO_Rep1" = getHeatSamp(
    sample_label_TKO_Rep1
  ),

  "TKO_Rpb1_Rapamycin_120_min" = getHeatSamp(
    sample_label_TKO_Rpb1_Rapamycin_120_min
  ),

  "TKO_Rpb1_Ino80_Rapamycin_120_min_Rep1" = getHeatSamp(
    sample_label_TKO_Rpb1_Ino80_Rapamycin_120_min_Rep1
  )
)
...
heatmap_upper_limit <- as.numeric(
  quantile(
    heatmap_values,
    probs = 0.99,
    na.rm = TRUE
...
ggsave(
  "combined_TES_ggheatmap_WT_TKO_4samples.png",
  plot = final_plot,
  width = 16,
  height = 9,
  dpi = 300
...
getHeatSamp2 <- function(name) {
  ...
  rownames(heat) <- make.unique(
    as.character(geneList$name)
  )
...
df_summary$position <- as.numeric(
  df_summary$position
)
...
# =========================================================
# REGULARITY QUARTILE INDIVIDUAL PROFILES
# =========================================================

prepare_reg_df <- function(quartile_label) {
  df_out <- df_summary %>%
    filter(regularity == quartile_label) %>%
    filter(sample %in% c(
      "Wild_type_Rep1",
      "TKO_Rpb1_Rapamycin_120_min",
      "TKO_Rpb1_Ino80_Rapamycin_120_min_Rep1"
...
reg_sample_colors <- c(
  "WT" = reg_wt_color,
  "TKO - Pol II" = "#E6C619",
  "TKO - Pol II - INO80" = "#3B6DBA"
)

reg_sample_lty <- c(
  "WT" = "solid",
  "TKO - Pol II" = "solid",
  "TKO - Pol II - INO80" = "solid"
)

plot_reg_profile <- function(df_reg, orient, quartile_label) {
  df_orient <- df_reg %>%
    filter(geneOrientation == orient)

  p <- ggplot() +
    geom_ribbon(
      data = df_orient %>% filter(sample == "WT"),
      aes(
        x = position,
        ymin = 0,
        ymax = mean_nucOccupancy
      ),
      fill = reg_wt_color,
      alpha = 1,
      color = NA
    ...
    scale_color_manual(
      values = reg_sample_colors,
      name = NULL
    ) +
    scale_linetype_manual(
      values = reg_sample_lty,
      name = NULL
    ) +
    scale_x_continuous(
    ...
    ylab("nucleosome occupancy") +
    xlab("distance from T-1") +
    ggtitle(paste0(orient, " - ", quartile_label)) +
    theme(
      plot.title = element_text(
        size = 11,
        face = "bold",
...
df_q4 <- prepare_reg_df("regularity_Q4")
df_q2 <- prepare_reg_df("regularity_Q2")
df_q1 <- prepare_reg_df("regularity_Q1")

p_q4_convergent <- plot_reg_profile(df_q4, "convergent", "regularity Q4")
p_q4_isolated <- plot_reg_profile(df_q4, "isolated", "regularity Q4")
p_q4_tandem <- plot_reg_profile(df_q4, "tandem", "regularity Q4")

p_q2_tandem <- plot_reg_profile(df_q2, "tandem", "regularity Q2")
p_q1_tandem <- plot_reg_profile(df_q1, "tandem", "regularity Q1")

ggsave(
  "reg_score_Q4_convergent.png",
  plot = p_q4_convergent,
  width = 6,
  height = 4,
  dpi = 300
)
